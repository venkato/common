...
//log.debug "wrong args ${args.expressionTypes}"
        ...
        PsiType element1Type = element1.getType()
        if (element1Type instanceof PsiClassType) {
//            PsiClassType jvmReferenceType = (PsiClassType) element1Type;
//            PsiClass psiClass = jvmReferenceType.resolve()
            String qualifiedName1 = element1Type.getCanonicalText()
            if (qualifiedName1 == 'net.sf.jremoterun.utilities.classpath.ClRef') {
                PsiClass psiClass1 = MyAcceptJrrProviderImpl.resolveClRef3(element1)
                log.info "rc ref resolved ? ${psiClass1 != null}"
                return psiClass1
            }
        }
        PsiElement varRef = element1.getSameNameVariants()[0].getElement();
        if (!(varRef instanceof GrVariable)) {
            log.debug "not a GrVar : ${varRef.getClass()} ${varRef}"
            return null;
        }
//        GrVariable grVariable = varRef as GrVariable
    ...
    private void testNotUsed() {
        ...
        JrrJavassistUtils.findMethodByCount(clazz, null, "", 1);

        Class class2 = com.intellij.idea.IdeaLogger;
        ClRef clRef2 = new ClRef('com.intellij.idea.IdeaLogger')
        JrrJavassistUtils.findMethodByCount(class2, null, "logErrorHeader", 1);
        JrrJavassistUtils.findMethodByCount(clRef2, null, "logErrorHeader", 1);
        JrrJavassistUtils.findMethodByCount(this.getClass(), null, "findPsiClass2", 1);
    }
