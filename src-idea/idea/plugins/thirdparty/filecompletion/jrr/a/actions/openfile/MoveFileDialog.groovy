...
/**
 * Do 2 thing at the same time :
 * 1. Rename file(or dir) on file system
 * 2. and change file name in field value
 */
@CompileStatic
...
//        panel.add(panelSub,BorderLayout.CENTER)
    ...
    void moveFileDo(ActionEvent e){
        try {
            ...
            net.sf.jremoterun.utilities.JrrUtilitiesShowE.showException("Failed move file",e2)
        }
    }
    ...
    void moveFileDoImpl(){
        ...
        String text4 = toSubDir.getText().replace('\\','/');
        File destFile
        if(parentFile==null) {
            destFile = text4 as File
        }else{
            assert parentFile.exists()
            destFile = new File(parentFile, text4)
        }
        ...
        String import2
        if(className3.length()==0){
            newText = """ new File ( "${text4}" ) """
        }else{
            import2 = "${packageName.getText()}.${className3}"
            newText = """ new File ( ${className3}.${fieldName} , "${text4}" ) """
        }
        ...
        if(offset7==-1){
            throw new Exception("failed find text4 : ${text1}")
        }
