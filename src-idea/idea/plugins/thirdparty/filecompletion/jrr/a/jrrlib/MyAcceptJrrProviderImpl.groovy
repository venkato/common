...
//                log.info "gr ref type"
        ...
        if (parent2 instanceof GrNewExpression) {
            ...
            if (text4.contains(MethodRef.getSimpleName())) {
                completionBean.methodName = ReflectionElement.newMethodRef
            } else if (text4.contains(FieldRef.getSimpleName())) {
                completionBean.methodName = ReflectionElement.newFieldRef
            } else {
            ...
            if (text1?.contains(MethodRef.getSimpleName())) {
                ...
                completionBean.methodName = ReflectionElement.valueOf(referenceExpression.getReferenceName());
            }
        }
    ...
    private void testNotUsed() {
        ...
        JrrClassUtils.getFieldValue(clRef1, "bound")
        JrrClassUtils.findField(clRef1, "bound")
        JrrClassUtils.findField(new ClRef('java.net.Socket'), "bound")
        JrrClassUtils.getFieldValue(net.sf.jremoterun.utilities.nonjdk.idea.IdeaRedefineClassloaderSupport.ivyDepResolver, "log")
        JrrClassUtils.getFieldValue(net.sf.jremoterun.utilities.nonjdk.idea.IdeaRedefineClassloaderSupport.redefineIdeaClassLoader, "ideaPluginId")
        // justRandomComment
        JrrClassUtils.getFieldValue(this, "log")
        JrrClassUtils.getFieldValueR(clRef1, this, "bound")
        JrrClassUtils.findField(this.getClass(), "log")
        JrrClassUtils.findField(clRef1, "bound")
        //JrrClassUtils.findMethod(testVar, "connect",2)
        JrrClassUtils.invokeJavaMethod(clRef1, "close")
        JrrClassUtils.invokeJavaMethod(testVar, "close")
        JrrClassUtils.invokeJavaMethod(testVar, "connect", null)
        JrrClassUtils.invokeJavaMethodR(clRef1, this, "connect", null)
        JrrClassUtils.invokeJavaMethod(clRef1, "connect", null)
        JrrClassUtils.invokeJavaMethod(new ClRef('java.net.Socket'), "connect", null)

        new FieldRef(testVar.getClass(), "bound");
        new FieldRef(clRef1, "bound");
        new MethodRef(testVar.getClass(), "close", 0)
        new MethodRef(testVar.getClass(), "connect", 1)
        new MethodRef(clRef1, "close", 0)
    }
