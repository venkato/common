...
//            log.info "enumCheck = ${enumCheck}"
    ...
    static boolean checkEnum(GrEnumConstantImpl e) {
        ...
        GrClassReferenceType type1
        boolean found = false
        if(!found) {
            type1 = grClassReferenceTypes.find { it.getName() == MavenIdContains.getSimpleName() }
             found = type1 != null
        }
        if(!found){
        ...
        if(!found){
            type1 = grClassReferenceTypes.find { it.getName() == MavenIdAndRepoCustomSourceContains.getSimpleName() }
            found = type1 != null
        }
    ...
    static enum notUsedEnum implements MavenIdContains {
        aa('cglib:cglib-nodep:3.2.4'),
        ideaClion('com.jetbrains.intellij.clion:clion:2019.2.4:zip'),
