...
public class MavenCompletionProviderImpl extends CompletionProviderCommon {
    ...
    private HashSet<String> cachedGroupsIds
    ...
    public static List<String> findMavenGroupIds(MavenCommonUtils mavenCommonUtils) {
        ...
        mdf.mavenFileFinder.getMavenLocalDir2().eachFileRecurse(FileType.FILES) {
            if (it.name.endsWith('.jar')) {
                groupsIds.add(it.parentFile.parentFile.parentFile)
            }
        }
        return groupsIds.collect {
            ClassPathCalculatorAbstract.getPathToParent(mdf.mavenFileFinder.getMavenLocalDir2(), it).replace('/', '.')
        }
    }
        ...
        switch (tokenCount) {
            case 0:
                ...
                if (lastCheckDateGroupsIds != null && System.currentTimeMillis() - lastCheckDateGroupsIds.getTime() < 60_000) {
                    ...
                    if (mdf.gradleFileFinder.getMavenLocalDir2()!=null && mdf.gradleFileFinder.getMavenLocalDir2().exists()) {
                        List<String> resGradle = mdf.gradleFileFinder.getMavenLocalDir2().listFiles().findAll {
                            it.isDirectory()
                        }.collect { it.name }
                        res.addAll(resGradle)
                        log.debug "res size = ${resGradle.size()}"
                    }
                    if (mdf.grapeFileFinder.getMavenLocalDir2()!=null && mdf.grapeFileFinder.getMavenLocalDir2().exists()) {
                        List<String> resGrape = mdf.grapeFileFinder.getMavenLocalDir2().listFiles().findAll {
                            it.isDirectory()
                        }.collect { it.name }
                        log.debug "res size = ${resGrape.size()}"
                        res.addAll resGrape
                    }
                    if (IdeaJavaRunner2Settings.suggestGrodupMavenIds && mdf.mavenFileFinder.getMavenLocalDir2().exists()) {
                        res.addAll(findMavenGroupIds(mavenCommonUtils));
                    }
                    cachedGroupsIds = new HashSet<String>(res)
            ...
            case 1:
                ...
                if (mdf.mavenFileFinder.getMavenLocalDir2().exists()) {
            ...
            case 2:
                ...
                if (mdf.gradleFileFinder.getMavenLocalDir2()!=null && mdf.gradleFileFinder.getMavenLocalDir2().exists()) {
                    File groupDir = new File(mdf.gradleFileFinder.getMavenLocalDir2(), groupId + '/' + artifact);
                    if (groupDir.exists()) {
                ...
                if (mdf.grapeFileFinder.getMavenLocalDir2()!=null && mdf.grapeFileFinder.getMavenLocalDir2().exists()) {
                    File groupDir = new File(mdf.grapeFileFinder.getMavenLocalDir2(), groupId + '/' + artifact);
                    if (groupDir.exists()) {
                ...
                if (mdf.mavenFileFinder.getMavenLocalDir2().exists()) {
                    File groupDir = new File(mdf.mavenFileFinder.getMavenLocalDir2(), groupId.replace('.', '/') + '/' + artifact);
                    if (groupDir.exists()) {
                        res.addAll(groupDir.listFiles().findAll {
                            it.isDirectory()
    ...
    static List<String> findAllArtifactForGroupInGradle(String groupId) {
        File groupDir = new File(mdf.gradleFileFinder.getMavenLocalDir2(), groupId);
    ...
    static List<String> findAllArtifactForGroupInGrape(String groupId) {
        File groupDir = new File(mdf.grapeFileFinder.getMavenLocalDir2(), groupId);
        if (!groupDir.exists()||!groupDir.directory) {
            return []
        }
        return groupDir.listFiles().findAll {
    ...
    static List<String> findAllArtifactForGroupInMaven(String groupId) {
        File groupDir = new File(mdf.mavenFileFinder.getMavenLocalDir2(), groupId.replace('.', '/'));
        if (!groupDir.exists()) {
            return []
        }
