...
class IdeaLibManagerSwing {
    ...
    public static ToFileRef2 ideaClassPathLatest =new  FileToFileRef( new File(  net.sf.jremoterun.utilities.groovystarter.JrrStarterVariables2.getInstance().filesDir,"idea_classpath_files_latest.groovy"));
    ...
    public LibManager3 libManager3 = new LibManager3(this);
...
//        editor.setMinimumSize(size2)
    ...
    void refreshLibs() {
        ...
        if (true) {
            ...
            List<LibItem> collect = libraries.collect { new LibItem(it,null) }
            collect = collect.sort()
            collect.collect { new DefaultMutableTreeNode(it) }.each { globalLibNode.add(it) }
        }
        if (true) {
            Project openedProject  = OSIntegrationIdea.openedProject
            LibraryTable libraryTable = registrar.getLibraryTable(openedProject);
            List<Library> libraries = libraryTable.getLibraries().toList();
            List<LibItem> collect = libraries.collect { new LibItem(it,openedProject) }
            collect = collect.sort()
...
//        editSettings.setEnabled(!SettingsRef.locationEdit.exists())
    ...
    private saveSettingsImpl() {
        ClassChecker2.classChecker2.analize2(SettingsRef.locationEdit.resolveToFile().text)
