...
class FieldResolvedDirectlyMoreComplex {
    ...
    ChildFileLazy tryResolveFileViaJavaRef(PsiReferenceExpression grReferenceExpression) {
        ...
        PsiType type = ((PsiReferenceExpression)firstChild).getType();
        return commonEnum(type, grReferenceExpression);
    }

    ChildFileLazy commonEnum(PsiType type, PsiElement psiElement) {
    ...
    ChildFileLazy resolveViaJavaExpression(String className,String expression){
        ...
        if (evaluate instanceof ChildFileLazy) {
            return (ChildFileLazy)evaluate
        }
        if (evaluate instanceof File) {
            File f = (File) evaluate;
            log.info "resolved ${expression} to ${f}"
            return new FileToFileRef(f);
        }
    ...
    ChildFileLazy tryResolveFileViaGrRef(GrReferenceExpression grReferenceExpression) {
        PsiElement[] children = grReferenceExpression.getChildren()
        if (children == null) {
            log.info("no childs")
            return null
    ...
    ChildFileLazy tryResolveFile(GrField varRef) {
