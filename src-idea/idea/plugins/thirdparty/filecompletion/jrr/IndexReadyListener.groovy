...
import net.sf.jremoterun.utilities.nonjdk.timer.javacore.scheduler.Scheduler
...
class IndexReadyListener {
            ...
            try {
                Runnable r = {
                    try {
                        ...
                        net.sf.jremoterun.utilities.JrrUtilitiesShowE.showException("failed run", e)
                    }
                }
    ...
    public static Runnable waitTimeout1 = {
        net.sf.jremoterun.utilities.JrrUtilitiesShowE.showException("no one project still opened 1", new Exception("no one project still opened at ${new Date()}"));
    };
    ...
    public static Runnable waitTimeout2 = {
        net.sf.jremoterun.utilities.JrrUtilitiesShowE.showException("no one project still opened 2", new Exception("no one project still opened at ${new Date()}"));
    };

    static LoadingState getState2() {
        AtomicReference<LoadingState> state
//        try {
//            state = JrrClassUtils.getFieldValue(StartUpMeasurer, 'currentState') as AtomicReference<LoadingState>  //NOFIELDCHECK
//        }catch (NoSuchFieldException e){
        Object state1 = JrrClassUtils.getFieldValue(LoadingState, 'currentState')
        if (state1 instanceof Supplier) {
            Supplier supplier = (Supplier) state1;
            state = (AtomicReference<LoadingState>)supplier.get()
        } else {
            state = state1 as AtomicReference<LoadingState>
        }
//        }
        return state.get()
    }
        ...
        if (listeners.size() == 0) {
            ...
            SwingUtilities.invokeLater {
                try {
                    ...
                    net.sf.jremoterun.utilities.JrrUtilitiesShowE.showException("failed run ${first}", e)
                }
            }
        }
    }
