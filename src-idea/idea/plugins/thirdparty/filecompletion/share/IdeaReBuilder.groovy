...
class IdeaReBuilder {
    ...
    public final long maxWait = DurationConstants.oneHour.timeInMsLong
    ...
    void rebuild(AfterResultReady afterResultReady) {
        synchronized (lock) {
            SwingUtilities.invokeLater {
                try {
                    rebuildImpl(afterResultReady)
                } catch (Throwable e) {
                    exception = e
                    log.warn("build failed", e);
                    net.sf.jremoterun.utilities.JrrUtilitiesShowE.showException("build failed", e);
                    resultReady()
                }
    ...
    void rebuildImpl(AfterResultReady afterResultReady) {
