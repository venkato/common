...
import net.sf.jremoterun.utilities.nonjdk.str2obj.v2.CommonToString2;
...
public class ServerUtilsStatic {
	...
	private static void mapping2() throws SecurityException,
		...
		if (primitiveClassToWrapperMapping.size() == 0) {
			...
			log.fine(""+primitiveClassToStringMapping);
		}
	}
	static {
		try {
	...
	public static byte[] handleInvokation(TcpSession tcpSession,
		...
		log.warning("received mi tid="+Thread.currentThread().getId()+" m="+ requestBean.methodName);
		try {
			...
			if(seriveObject.service instanceof NoCreateNewThread){
				handleInvokation9(tcpCallInfo);
			}else {
	...
	public static void handleInvokation(TcpCallInfoServer callInfo)
			throws Exception {
//		log.info("tid="+Thread.currentThread().getId()+" m="+ callInfo.method.getName());
				...
				.size()];
		{
			...
			for (String className : callInfo.requestBean.classesNameParams) {
				Class loadClass = PrimiteClassesUtils.loadPrimitiveClass(className);
//				PrimiteClassesUtils.primitiveClassesArraysShortName.indexOf(className);
				if (loadClass == null) {
	...
	public static void handleInvokationNewThread(
		...
		log.fine(""+callInfo.tcpSession);
		Thread thread = new Thread() {
			@Override
			public void run() {
				handleInvokation9(callInfo);
	...
	public static void handleInvokation9(			final TcpCallInfoServer callInfo) {
				try {
					...
					log.log(Level.WARNING, addString, e);
				}
	}
	...
	public static void writeResponse(ReponseBean reponseBean,
		...
		log.fine("writing "+new String(res));
		try {
