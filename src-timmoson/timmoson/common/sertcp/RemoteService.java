...
//			.getBytes();
			...
			while (true) {
				...
				if (readBytes == -1) {
					...
					try {
						...
						if(writeStrangeBytes) {
							log.log(Level.INFO,"test call", e);
						}
					}
				} else {
	...
	public byte[][] findBetween(String begin, String end,
		...
		log.fine(""+beginI);
		log.fine(""+endI);
		log.fine(new String(bb, beginI, diff + 1, TimmosonSettings.defaultEncoding));
		System.arraycopy(bb, beginI - 1, bb2, 0, bb2.length);
		log.fine(new String(bb2));
		...
		bbbw[1] = remain;
		log.fine(remain.length + " " + new String(remain));
		return bbbw;
