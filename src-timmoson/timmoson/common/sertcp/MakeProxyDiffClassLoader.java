...
public class MakeProxyDiffClassLoader {
	...
	public static Object findSession() throws Exception {
		...
		Object session = JrrClassUtils.invokeJavaMethodR(new ClRef("timmoson.common.CallInfoServer"),callInfoServer, "getSession");
		return session;
	}

	public static <T> T makeClient(Class<T> class1, String serviceId) throws Exception {
		Object session = findSession();
		T t = (T) JrrClassUtils.invokeJavaMethodR( new ClRef("timmoson.common.CallBackSession"), session, "makeClient", class1, serviceId);
		return t;
	}
