...
import net.sf.jremoterun.utilities.nonjdk.log.tojdk.threadfilter.Log4jAppenderListener;
...
public abstract class InvokactionRemoreRun extends JrrCommonLauncher {
	...
	protected net.sf.jremoterun.utilities.nonjdk.log.tojdk.threadfilter.IfAppender appenderListenerJdk;
	...
	protected void jrrInitLoggerTransferer() throws Exception {
            ...
            appenderListenerJdk = new net.sf.jremoterun.utilities.nonjdk.log.tojdk.threadfilter.IfAppender(session);
            appenderListenerJdk.loggingLogger.add(getClass().getName());
            JdkLogFormatter.getRootLogger().addHandler(appenderListenerJdk);
	}
			...
			Thread thread = new Thread() {
				...
				public void run() {
					try {
						...
						session = TcpSession.buildTcpSession(new SocketWrapper(socket));
	...
	public static void registerServices() {
            ServiceLocator.regNewService(Log4jAppenderListener.class, new net.sf.jremoterun.utilities.nonjdk.log.tojdk.threadfilter.Log4jAppenderListener());
	}
	...
	protected void jrrDeInitLoggerTransferer() {
		if (appenderListenerJdk != null) {
            JdkLogFormatter.getRootLogger().removeHandler(appenderListenerJdk);
        }
...
//		}
	...
	public void log(Object msg) {
		...
		sb.append(msg);
            ...
            appenderListenerJdk.appenderListener.add(sb.toString());
