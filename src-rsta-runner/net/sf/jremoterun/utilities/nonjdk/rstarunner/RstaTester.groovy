...
class RstaTester extends TestCase {
    ...
    void test2() {
        ...
        SwingUtilities.invokeLater {
            ...
            log.info "${1}"
            TabWindow notebook1 = tcpmon.getNotebook()
            JFrame frame = SimpleFrameCreator.createAppFrameIdw("RstaTester", notebook1)
            log.info "${2}"
            notebook1.addTab(new View("SimpleLabel", null, new JLabel("simple label")))
            IdwWindowFinder finder = new IdwWindowFinder()
            notebook1.addTab(finder.view)
//        finder.view.undock(new Point(100,100))
            frame.setVisible(true)

            //IdwUtils.setVisible(tcpmon.adminView)
        }
        Thread.sleep(Long.MAX_VALUE)
    }
