...
//    public int runBigQueryEvery = 10;
    ...
    public Map<List<Long>, MergeRequest> mapObject = new HashMap<>()
...
//            filter.setSimpleView(true)
            ...
            gitlabMrMonitorHelper.onList(mergeRequests)
            lastTimeToCheck = lastTimeToCheck2
            queryTimeToExecute = System.currentTimeMillis() - lastTimeToCheck2.getTime();
        }
    }

    public GitlabMrMonitorHelper gitlabMrMonitorHelper=new GitlabMrMonitorHelper(this)

    void gotMergeRequest2(MergeRequest mr) {
        List<Long> key = [mr.getProjectId(), mr.getIid()]
        mapObject.put(key, mr)
    }

    void gotMergeRequest(MergeRequest mr) {
        synchronized (lock) {
            List<Long> key = [mr.projectId, mr.iid]
    ...
    void onMrUpdatedUserNotes(MergeRequest mrOld, MergeRequest mrNew) {
        ...
        if (diff > 0) {
            ...
            if(size - diff<0){
                throw new IllegalStateException("discussionSize = ${size} oldNotesCount=${mrOld.getUserNotesCount()} newNotesCount=${mrNew.getUserNotesCount()} ")
            }
