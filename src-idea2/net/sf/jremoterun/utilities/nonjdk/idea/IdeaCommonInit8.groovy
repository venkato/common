...
class IdeaCommonInit8 implements Runnable {
    ...
    static void init1Impl() {
        ...
        if(setLog4jFactory) {
            ...
            if (setLog4jFactory2) {
                if(!JdkLoggerSwitch.useJdkLogger) {
                    RunnableFactory.runRunner new ClRef('net.sf.jremoterun.utilities.nonjdk.log.log4j2.Log4j2FactorySetter')
                }
            }
        }
        ...
        log.info "cp1"
        InitGeneral.init1()
        if(net.sf.jremoterun.utilities.nonjdk.log.tojdk.JdkLoggerSwitch.useJdkLogger){
