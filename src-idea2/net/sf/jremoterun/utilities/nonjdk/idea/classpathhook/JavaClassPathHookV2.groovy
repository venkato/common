...
import net.sf.jremoterun.utilities.nonjdk.shellcommands.javarunner.versiondetector.JavaVersionDetector
...
class JavaClassPathHookV2 extends InjectedCode {
    ...
    static void installHookV2() {
        Class clazz = com.intellij.execution.configurations.JavaCommandLineState
        CtClass ctClass = JrrJavassistUtils.getClassFromDefaultPool(clazz)
        CtMethod method = JrrJavassistUtils.findMethodByCount(clazz, ctClass, "prepareTargetEnvironmentRequest", 3);
        //NOFIELDCHECK
        method.insertAfter """
            ${CodeInjector.createSharedObjectsHookVar2(clazz)}
...
//    public static boolean useNew = true
    ...
    String getValueJrr(TargetValue<String> elI) {
...
//        list
        //SharedObjectsUtils.buildDefauldSharedObjects()
//        Map mmm = (Map)Runner.sharedObjects.get('debugIdea1')
//        if(mmm==null){
//            mmm=[:]
//            Runner.sharedObjects.put('debugIdea1',mmm)
//        }
//        mmm.put(new Date(),[key,getValueJrr(generalCommandLine.getExePath())])
        List<String> optionsJaveVer
        String javaPath = getValueJrr(generalCommandLine.getExePath())//getValueJrr(parametersList[0])
        boolean booleanIs8 = JavaVersionDetector.javaVersionDetector.detectIs8WithCache(javaPath)
        if (booleanIs8) {
            optionsJaveVer = IdeaJavaRunner2Settings.jvmJava8Options
        } else {
            optionsJaveVer = IdeaJavaRunner2Settings.jvmJava11Options
        }

        optionsJaveVer.reverse(false).each {
