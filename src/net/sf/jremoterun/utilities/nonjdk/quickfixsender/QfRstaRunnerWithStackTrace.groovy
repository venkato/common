...
class QfRstaRunnerWithStackTrace extends RstaRunnerWithStackTrace2 {
    ...
    JButton showObjectButton = new JButton("Inspect")
    SplitWindow splitPaneRight;
    SplitWindow splitPaneQf;
    QfRunnerType runnerType;
    public QfRstaRunnerWithStackTrace thisObjectRef = this
...
//        panelButtons.add(customButton)
        ...
        if(EnvOpenSettings.defaultShowObject!=null){
            showObjectButton.addActionListener {new ActionListener(){

                @Override
                void actionPerformed(ActionEvent e) {
                    EnvOpenSettings.defaultShowObject.showObject(thisObjectRef)
                }
            }}
        }
...
//        rightPanel.addTab(msgToSendView.view)
        ...
        viewMsgToSendView.textArea.setSyntaxEditingStyle(SyntaxConstants.SYNTAX_STYLE_XML)
        doLayoutQf()
    }
    ...
    void showMsg(int activeColumn) {
        Vector row = (Vector)defaultTableModel.getDataVector().get(activeColumn)
    ...
    void showMsgAsXml(String msg) {
        msg = msg.replace(d.humanSep, net.sf.jremoterun.utilities.nonjdk.quickfixsender.QuickFixMsgUtils.fixMsgNativeSep)
        ...
        viewMsgToSendView.textArea.setText(asXml)
    }
...
//        textArea.removeAllLineHighlights();
    ...
    void showException(Throwable e) {
        ...
        viewMsgToSendView.textArea.setText(excS)
    ...
    void addMessageToView(Message msg, boolean isOutMsg) {
        try {
            if (jrrQfHelper.isNeedAddMsgToTable(msg, isOutMsg)) {
                ...
                row.add msg.toString().replace(net.sf.jremoterun.utilities.nonjdk.quickfixsender.QuickFixMsgUtils.fixMsgNativeSep, d.humanSep);
