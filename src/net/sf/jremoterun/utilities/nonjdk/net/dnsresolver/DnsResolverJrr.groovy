...
class DnsResolverJrr {
    ...
    static boolean isIpAddr(String host) {
        List<String> tokenize = host.tokenize('.')
        if (tokenize.size() != 4) {
            return false
        }

        String nonInt = tokenize.find { !it.isInteger() }
        return nonInt == null
    }
    ...
    List<ARecord> resolveHostNames(String hostName) {
        Name hostName2 = Name.fromString(hostName)
        LookupResult lookupResult = s.lookupAsync(hostName2, Type.A).toCompletableFuture().get()
        List<Record> records = lookupResult.getRecords()
        return (List)records
    }

    ARecord resolveHostName(String hostName) {
        Name hostName2 = Name.fromString(hostName)
        LookupResult lookupResult = s.lookupAsync(hostName2, Type.A).toCompletableFuture().get()
        List<Record> records = lookupResult.getRecords()
        assert records.size() == 1
        ARecord aRecord = records[0] as ARecord
        //String aRecord.getName().toString()
        return aRecord
    }
