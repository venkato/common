...
import net.sf.jremoterun.utilities.nonjdk.net.proxynew.ContainsStringDecision
...
class ProxyTrackerStat implements ProxyTrackerI, DefaultObjectName {
    ...
    public ContainsStringDecision tracedHosts = new ContainsStringDecision()
    public ContainsStringDecision rejectedHosts = new ContainsStringDecision()
...
//                if (tracedHosts.contains(host4)) {
                if (tracedHosts.isMatched(host4)) {
...
//                if (rejectedHosts.contains(host4)) {
                if (rejectedHosts.isMatched(host4)) {
                    return false
                }
                return true
            }
