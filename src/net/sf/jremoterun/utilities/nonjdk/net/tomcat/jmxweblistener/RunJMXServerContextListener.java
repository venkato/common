...
public class RunJMXServerContextListener implements ServletContextListener {
    ...
    public static String userS = "userJmx";
    public static String passwordS = "passwordJmx";
    ...
    public void contextInitialized(final ServletContextEvent arg0) {
        ...
        if (jmxPort2 == null) {
            ...
            user = arg0.getServletContext().getInitParameter(userS);
            password = arg0.getServletContext().getInitParameter(passwordS);
            final ClassLoader classLoader = Thread.currentThread()
                    .getContextClassLoader();
            SimpleFindParentClassLoader.setDefaultClassLoader(classLoader);
            if (server == null || !server.isActive()) {
                try {
                    server = JrrUtils.creatJMXConnectorAndRMIRegistry(JrrUtils.findLocalMBeanServer(), rmiRegistryPort, user, password);
