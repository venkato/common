...
public class Log4j1PatternLayout extends Layout {
	...
	public void formatImpl(LoggingEvent logRecord, StringBuilder sb) {
	...
	public boolean isLevelWarningOrAbove(LoggingEvent logRecord){
		Level level1 = logRecord.getLevel();
		if(level1==null){
			return true;
		}
		return  level1.isGreaterOrEqual( Level.WARN) ;
		//return true;
	}

	public void logTime(LoggingEvent logRecord, StringBuilder sb) {
		if (isLevelWarningOrAbove(logRecord)) {
			sb.append(getTime());
			sb.append(" ");
	...
	public void logStackTrace(LoggingEvent logRecord, StringBuilder sb, StackTraceElement[] stackTraces) {
		boolean error = false;
		if (isLevelWarningOrAbove(logRecord)) {
			error = true;
		}
