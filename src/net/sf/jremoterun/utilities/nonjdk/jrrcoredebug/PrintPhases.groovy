...
class PrintPhases implements Runnable{
    ...
    void addDebug(JrrRunnerPhaseI phase){
        if( GroovyMethodRunnerParams.gmrp.seqPatternRunnerGmrp.isPhasePassed(phase)){

        }else{
            Runnable befoer1 = {
                log.info "before ${phase} ${GroovyMethodRunnerParams.gmrp.args} "
                if(phase== JrrRunnerPhase2.runTargetMethod){
                    printMainMethods()
                }
            }
            Runnable after1 = {
                log.info "after ${phase} ${GroovyMethodRunnerParams.gmrp.args}"
            }
            GroovyMethodRunnerParams.gmrp.seqPatternRunnerGmrp.addL(phase,true,befoer1)
            GroovyMethodRunnerParams.gmrp.seqPatternRunnerGmrp.addL(phase,false,after1)
        }
    ...
    @Override
    void run() {
        new PrintPhases().addDebug (JrrRunnerPhase.begin)
