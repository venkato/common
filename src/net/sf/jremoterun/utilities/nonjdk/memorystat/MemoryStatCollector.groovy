...
abstract class MemoryStatCollector implements DefaultObjectName {
    ...
    public long ignoreLongGcHappenedOlderThen = DurationConstants.oneHour.timeInMsLong
...
//        log.info "${gcInfoBeanLong.lastRun}"
        if (oldGen.usedPercent > maxUsedOldGenPercent) {
            if (gcInfoBeanLong.bean.getLastGcInfo() == null) {
                ...
                if (gcInfoBeanLong.lastRun.getTime() > System.currentTimeMillis() - ignoreLongGcHappenedOlderThen) {
                    String human1 = bytesToHumanConverter.convertHuman(oldGen.used)
                    String msg = "mem used ${oldGen.usedPercent} % ${human1} after big gc ${gcInfoBeanLong.lastRun}"
                    onMsg(msg)
                }
            }
        }
    ...
    void collectMemoryStat2() {
        ...
        beans.each {
            ...
            String human1 = bytesToHumanConverter.convertHuman(usage.used)
            log.info "Memory ${it.name} usage : ${human1}"
        }
