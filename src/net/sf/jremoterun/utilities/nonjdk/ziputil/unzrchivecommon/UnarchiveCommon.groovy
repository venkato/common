...
class UnarchiveCommon {
    ...
    void doJob() {
        ArchiveEntry lastReadItem
        try {
            while (true) {
                ArchiveEntry nextEntry = archiveInputStream.getNextEntry()
                lastReadItem = nextEntry
                if (nextEntry == null) {
                    break
                }
            ...
            if(lastReadItem==null){
                log.info "failed, last read unknown"
            }else {
                log.info "failed, last read ${lastReadItem.getName()}"
            }
            throw e
        } finally {
            onFinally()
...
//        if (errorEntries.size() > errorEntriesMax) {
//            throw e
//        }
        throw e
    }
