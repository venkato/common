...
//        if(inSec>2_147_483_647){
    ...
    Date moveDateFromNow(long count) {
        long newValue = count * timeInMsLong
        return new Date(newValue + System.currentTimeMillis())
    }

    Date moveDate(long count, Date date1) {
        long newValue = count * timeInMsLong
        return new Date(newValue + date1.getTime())
    }
}
