...
@CompileStatic
class JavaClassNameChildPattern implements ChildChildPattern {
    private static final Logger log = JrrClassUtils.getJdkLogForCurrentClass();

    public List<String> suffixes = ['.class','.groovy','.java'];
    ...
    File resolveChild(File parent) {
        ...
        suffixes.each {
            if(f2==null) {
                File f1 = new File(pakc, classNameLast + it)
                if (f1.exists()) {
                    f2 = f1
                }
            }
