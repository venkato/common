...
import net.sf.jremoterun.utilities.nonjdk.javalangutils.version.JavaVersionMapping
...
//    AddFilesToJavasistPool classPool = new AddFilesToJavasistPool(new ClassPool());
    ...
    static String normalizeClassName(String className) {
        if (className.contains('\\') && className.endsWith(ClassNameSuffixes.dotclass.customName)) {
            className = className.replace('\\', '/')
        } else if (!className.contains('/') && className.contains('.')) {
            className = className.replace('.', '/') + ClassNameSuffixes.dotclass.customName
        }
        return className
    ...
    ClassFile getClassFile2(File jarFile, String className) {
        ...
        try {
            ...
            JrrIoUtils.closeQuietly2(zipFile, log)
        }
    }
    ...
    ClassFile getClassFile(File classFile) {
        ...
        try {
            ...
            JrrIoUtils.closeQuietly2(dataInputStream, log)
        }
    }
