...
class JrrConfigGenerator implements ClassNameSynonym{
    ...
    static  String readText(String fileSuffix) {
        ...
        InputStream inputStream = JrrClassUtils.getCurrentClassLoader().getResourceAsStream(resourceName)
        if (inputStream == null) {
...
//    }
    ...
    void generateConfig(boolean raw, boolean userConfig, boolean overrideFile) {
        ...
        File jrrConfigDir = net.sf.jremoterun.utilities.groovystarter.JrrStarterVariables2.getInstance().createFilesDirIfNeeded()
        String className3
        File f
        if (raw) {
