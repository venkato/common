...
class JrrIdeaGenerator implements ClassNameSynonym {
    ...
    public ChildPattern ideaOptionsRef = IdeaFilesLayout.idea64ExeOptions.ref

    void updateVmOptions(File ideaPLuginDir) {
        File ideaVmOptions = ideaOptionsRef.resolveChild(ideaPLuginDir)
        assert ideaVmOptions.exists()
        String text = ideaVmOptions.text
        boolean needSave = false
        if (!text.contains(JrrStarterJarRefs2.jremoterun.name())) {
            String s = JavaAgentOptionBuilder.createAgentArg(JavaCmdOptions.javaagent, JrrStarterJarRefs2.jremoterun) + '\n'
            text += s
            needSave = true
        }
        if (!text.contains(JrrStarterJarRefs2.java11base.name())) {
            String s = JavaAgentOptionBuilder.createAgentArg(JavaCmdOptions.javaagent, JrrStarterJarRefs2.java11base) + '\n'
            text += s
            needSave = true
        }
        if (needSave) {
    ...
    void generateIdeaConfig(File gitRepo, File ideaLogDir, File compiledClasses) {
        ...
        File jrrConfigDir2 = net.sf.jremoterun.utilities.groovystarter.JrrStarterVariables2.getInstance().createFilesDirIfNeeded()
