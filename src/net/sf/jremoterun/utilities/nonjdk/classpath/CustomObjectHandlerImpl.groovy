...
class CustomObjectHandlerImpl implements CustomObjectHandler {
    ...
    CustomObjectHandlerImpl(File gitRepo) {
        ...
        File unzup111 =  new File(gitRepo, "unzip")
...
//                return resolveToFile(redirect)
            ...
            case { object instanceof GitBaseRef }:
                GitBaseRef gitSpec = (GitBaseRef) object;
                return cloneGitRepo3.getFileIfDownloaded(gitSpec)
    ...
    String resolvePathForRefApproximately(ToFileRef2 ref,boolean downloadForce) {
        if (ref == null) {
        ...
        if (ref instanceof SfLink) {
            return 'sf/'+ref.getPath()
        }
        if (ref instanceof RefLink) {
            return resolvePathForRefApproximately(ref.fileRef,downloadForce)
        }
        if (ref instanceof ToFileRefRedirect) {
            return resolvePathForRefApproximately(ref.getRedirect(),downloadForce)
        }
        ...
        if (ref instanceof UnzipRef) {
            if(downloadForce){
                throw new Exception("unsuppoertf ${ref}")
            }
            return 'unzip/'+ref.refToZip;
        }
        if (ref instanceof FileChildLazyRef) {
            String parent = resolvePathForRefApproximately(ref.parentRef,downloadForce)
            return parent + '/' + ref.child.approximatedName();
        }
        if (ref instanceof MavenIdAndRepoContains) {
            return resolvePathForRefApproximately(ref.getMavenIdAndRepo().m,downloadForce)
        }
...
//            return null
//        }
        // file resolved, so suffix should not be empty
