...
class GetClassesFromLocation {
    ...
    List<String> handleModuleJava11(File moduleFile) {
        ImageReader imageReader = ImageReader.open(moduleFile.toPath())
        try {
            List<String> entriess = imageReader.getEntryNames().toList().findAll { it.endsWith(ClassNameSuffixes.dotclass.customName) }.collect { UsedByAnalysis.removeDotClass(it) }
            return entriess
        }finally {
