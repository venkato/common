...
@CompileStatic
abstract class PreLoadClassesIde {
    ...
    public PreLoadClasses() throws Exception {
    }
    ...
    abstract ClassLoader findClassLoaderForPlugin(String pluginId)
        ...
        for (String pluginId : plugins) {
            try {
                if (!loadClassesByClassloaders.containsKey(pluginId)) {
                    ClassLoader findPluginModuleClassLoader = findClassLoaderForPlugin(pluginId);
    ...
    public boolean isPreloadPlugin(String pluginId, Collection<String> classes) {
        if (skipPlugins.contains(pluginId)) return false;
        if (classes.size() < minClassesToLoad) return false;
    ...
    public void preloadPlugin(String pluginId, Collection<String> classes) {
        ...
        ContextClassLoaderWrapper.wrap2(loadClassesByClassloader.classLoader1, {
            loadClassesByClassloaders.get(pluginId).loadClassesByLocation(classes);
        });
