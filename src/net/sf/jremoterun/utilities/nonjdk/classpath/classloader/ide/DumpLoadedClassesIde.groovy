...
import net.sf.jremoterun.utilities.nonjdk.io.byteshuman.MinBytesThreshold
...
class DumpLoadedClassesIde {
    ...
    public MinBytesThreshold freeSpaceNeed = new MinBytesThreshold(BytesDivider.mb.divider)
    public IdePluginsClassloaderInfo idePluginsClassloaderInfo

    public static List<String> containsIgnoreDumperS = ['$']

    DumpLoadedClassesIde(File file, long periodInSec, IdePluginsClassloaderInfo idePluginsClassloaderInfo,long startUpDelayInSec) {
        this.file = file
        this.periodInSec = periodInSec
        this.idePluginsClassloaderInfo = idePluginsClassloaderInfo
        this.startUpDelayInSec = startUpDelayInSec
    }
    ...
    void dumpToFile() {
        ...
        if (freeSpaceNeed !=null) {
            File parentFile = file.getParentFile()
            long spaceNow = parentFile.getFreeSpace()
            if (freeSpaceNeed.isLower(spaceNow)) {
                String human1 = freeSpaceNeed.convertHuman(spaceNow)
                log.info("skip dump : space left on device not enough : ${human1} ${parentFile.getPath()}")
                continue1 = false
            }
        }
        if (continue1) {
