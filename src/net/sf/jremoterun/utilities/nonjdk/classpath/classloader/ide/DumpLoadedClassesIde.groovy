...
@CompileStatic
abstract class DumpLoadedClassesIde {
    ...
    public long freeSpaceNeed = 10_000_000; //10mb

    DumpLoadedClassesIde(File file, long periodInSec) {
        this.file = file
    ...
    void startClassDumping(int rotateCount) {
        ...
        Runnable r = {
            ...
            try {
                synchronized (lock) {
                    lock.wait(startUpDelayInSec*1000)
                }
                while (needRun) {
                    dumpToFile()
                    synchronized (lock) {
    ...
    abstract Collection<String> getPlugins()
    ...
    public StoreStat dumpLoadedClassesNames() throws Exception {
        ...
        Collection<String> pluginSymbolicNames = getPlugins();
