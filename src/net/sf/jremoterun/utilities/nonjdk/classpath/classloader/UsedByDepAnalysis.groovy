    ...
    protected void getDependentClasses(String toAnalize, int maxDepth) {
        ...
        if (handle1) {
            ...
            if (location.f.isDirectory()) {
                File classFile = location.f.child(location.fullPathInJarSlash)
                if (!classFile.exists()) {
                ...
                if (location.f.getName() == net.sf.jremoterun.utilities.nonjdk.classpath.classloader.GetClassesFromLocation.modulesFileName) {
                    ImageReader imageReader = ImageReader.open(location.f.toPath())
                    try {
                        //classNamesOnLocationUsed22.each {
                            // works ? need add .class suffix ?
                            byte[] bytes = imageReader.getResource(location.fullPathInJarSlash)
                            if (bytes != null) {
                    ...
                    ZipEntryCallback zipEntryCallback = new ZipEntryCallback() {
                        ...
                        void process(InputStream inputStream, ZipEntry zipEntry) throws IOException {
                            if (zipEntry.getName() == location.fullPathInJarSlash) {
