...
//    static  prefOld;
    ...
    public static boolean doPrefCheck = true

    static void customCreateListPackages() {
        if(doPrefCheck) {
            // class removed from jrr groovy.jar, so add groovy jar from maven
             new ClRef('org.codehaus.groovy.tools.shell.util.Preferences').loadClass(JrrClassUtils.getCurrentClassLoader())
        }
