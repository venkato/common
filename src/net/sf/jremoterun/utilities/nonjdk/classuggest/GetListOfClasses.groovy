...
class GetListOfClasses {
    ...
    Class loadClassFromPackage(Class sampleClass, String className) {
        if (className == ConsoleSymbols.question.s || className == null) {
...
//            throw new ClassNotFoundException("Failed load ${className}, available ${classes}", e)
    ...
    List<String> getListOfClasses(File baseDir, String packageName) {
        ...
        if (baseDir.isDirectory()) {
            ...
            names = names.findAll { it.getName().endsWith(ClassNameSuffixes.dotgroovy.customName) || it.getName().endsWith(ClassNameSuffixes.dotclass.customName) }
            List<String> res = names.collect { it.getName().replace(ClassNameSuffixes.dotgroovy.customName, '').replace(ClassNameSuffixes.dotclass.customName, '') }
        ...
        res = res.collect { it.replace(ClassNameSuffixes.dotgroovy.customName, '').replace(ClassNameSuffixes.dotclass.customName, '') }
