...
class SSHClientJrr extends SSHClient {
    ...
    void connectJrr(SshConSet2 conSet2){
        ...
        }else if(conSet2.password!=null) {
            authPassword(conSet2.user,conSet2.password)
        }
    }
    ...
    NegotiatedAlgorithms getNegotiatedAlgorithms(){
        ...
        net.schmizz.sshj.transport.NegotiatedAlgorithms negotiatedAlgorithms = JrrClassUtils.getFieldValue(kexer,'negotiatedAlgs') as NegotiatedAlgorithms
...
//        Object negotiatedAlgorithms = JrrClassUtils.getFieldValue(kexer,'clientProposal')
    ...
    KeyExchange getKeyExchange(){
        ...
        new ClRef('net.schmizz.sshj.transport.KeyExchanger')
        Object kexer = JrrClassUtils.getFieldValue(transport1,'kexer')
        net.schmizz.sshj.transport.kex.KeyExchange keyExchange = JrrClassUtils.getFieldValue(kexer,'kex') as KeyExchange
        return keyExchange;
    }
...
//        log.info "negotiatedAlgorithms=${negotiatedAlgorithms1}, serverPublicKey=${algorithm} ${hostKey.getClass().getName()} ${base64String}"
        Map<String, Object> negotiatedDump1 = new ObjectDumperProvider( new JavaObjectPropsDumper()).dumpObject(negotiatedAlgorithms1) as Map
//        Map<String, Object> clientProposal =
        ...
        r.put(SshjDumpParams.proposalClient,new ObjectDumperProvider(new JavaObjectPropsDumper([SshProposalsParser.packetField])).dumpObject(getClientProposals()))
        r.put(SshjDumpParams.proposalServer,new ObjectDumperProvider(new JavaObjectPropsDumper([SshProposalsParser.packetField])).dumpObject(getServerProposals()))
