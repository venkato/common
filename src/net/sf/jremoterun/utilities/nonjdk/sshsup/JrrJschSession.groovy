...
import net.sf.jremoterun.utilities.nonjdk.io.LastByteArray2OutputStream
...
import net.sf.jremoterun.utilities.nonjdk.store.configloader.a2.TextConfigLoader
...
class JrrJschSession extends JrrJschSessionOriginal {
    ...
    public LastByteArray2OutputStream lastReceivedBytes2 =  new LastByteArray2OutputStream()
    public LastByteArray2OutputStream lastSentBytes2 =  new LastByteArray2OutputStream()
    ...
    void onOutputBuffer(Buffer buffer) {
        ...
        lastSentBytes2.reset()
        JschBufferHelper.writeToOutputStream(buffer,lastSentBytes2)
...
//                int k = string.indexOf(passwordS)
    ...
    Buffer read(Buffer buf) throws Exception {
        ...
        //int msgType=b.getCommand()&0xff;
        lastReceivedBytes2.reset()
        JschBufferHelper.writeToOutputStream(b,lastReceivedBytes2)
        if (isNeedLogRawMsg()) {
...
//    }
    ...
    Map<String, Object> fetchProposalsJson(FieldRef fieldName) {
        ...
        return (Map)  new ObjectDumperProvider( [SshProposalsParser.packetField]).dumpObject(obj)
    }
...
//        }
    ...
    void dumpMainParamsToFileImpl(File folder, String fileSuffix, int rotateCount, Map<KeyExchangeEnum1, Object> human1) {
        ...
        if (pass1) {
            String humanNew = JsonOutput.prettyPrint(JsonOutput.toJson(human1))


//            String params1 = human
//        String params1 =human.collect {"${it.key} = ${it.value}"}.join('\n')
            new TextConfigLoader(f,rotateCount).saveIfNeeded(humanNew)
        } else {
