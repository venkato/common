...
class SshAuthHandlerJrr {
    ...
    public volatile boolean dumpSessionParamsAfterAuth = JSchSettingsDefaults.dumpSessionParamsAfterAuthS
    ...
    boolean handle(SshAuthCallParentMethod handle, Session session) {
        ...
        try {
            ...
            if (b) {
                if (dumpSessionParamsAfterAuth) {
                    if (session instanceof JrrJschSession) {
                        JrrJschSession jrrSession = (JrrJschSession) session;
//                    log.info "dump params : ${jrrSession.fetchGuessHuman()}"

                        jrrSession.jrrSchSessionLog.logMsg("dump params : ${jrrSession.fetchGuessHumanS()}")
                    }
                }
