...
//    SshConSet sshConSet
    ...
    void get(String remotePath, File dest) {
        getFile(remotePath, dest)
    }

    /**
     *
     * @param src
     * @param remotePath must be dir
     */
    ...
    String findRemoteFile2(String remoteFile, int maxFoundCount) {
        ...
        int size = res.size()
        if (size == 0) {
            throw new SftpFailedFindFileException("not found : ${remoteFile}")
        }
        if (size > 1) {
            throw new SftpFailedFindFileException("found many : ${res.sort()}")
        }
        return res[0]
    }
    ...
    List<String> handleSubPath(final String pathParent, String el, List<String> subPaths, int maxFoundCount, boolean throwExc, boolean forceCd) {
        ...
        if (size == 0) {
            if (throwExc) {
                throw new SftpFailedFindFileException("no matches in ${pathParent} for ${el}")
            }
            return []
        }
...
//        return [path + el]
    ...
    Collection<ChannelSftp.LsEntry> lsAsterixGlob(String conextPath, String el){
        // need use proper method instead of : ls(el)
        Collection<ChannelSftp.LsEntry> ls = sftp.ls(el) as Collection<ChannelSftp.LsEntry>
        String perl5 = GlobCompiler.globToPerl5(el.toCharArray(), 0)
        Pattern pattern = Pattern.compile(perl5)
        Collection<ChannelSftp.LsEntry> mactheds = ls.findAll {pattern.matcher(it.getFilename()).matches()}
        if(mactheds.size()==0){
            List<String> collect1 = ls.collect { it.getFilename() }.sort()
            log.info("nothing found by ${el} , all items ${collect1.size()} : ${collect1.join(',')}")
        }
        return mactheds
    }

    Collection<ChannelSftp.LsEntry> lsAsterix(String conextPath, String el){
        Collection<ChannelSftp.LsEntry> ls = sftp.ls(el) as Collection<ChannelSftp.LsEntry>
        return ls
    }
    ...
    String findRemoteFileImpl(String remoteFile) {
        ...
        tokenize.each { String el ->
            ...
            if (el.contains("*")) {
                ...
                try {
                    Collection<ChannelSftp.LsEntry> ls = lsAsterix(path, el)
                    switch (ls.size()) {
                        case 0:
                            throw new SftpFailedFindFileException("Not found : ${path}/${el}")
                        case 1:
                            if (first) {
                                first = false
                            } else {
...
//                            path += ls.first().filename
                            pathToAdd += ls.first().filename
                            break
                        default:
                            throw new SftpFailedFindFileException("Many files for : ${path}/${el} : ${ls.collect { it.filename }.sort()})")
                    }
                } catch (Exception e) {
                    log.info "failed list in : ${path} for ${el}"
                    throw e
...
//                path += el
    ...
    void uploadFileOrDir(File local, String remoteDir, List<String> excludes) {
        assert local.exists()
        if (local.isDirectory()) {
            createDirSilent(remoteDir)
            local.listFiles().toList().each {
                if (!excludes.contains(it.getName())) {
                    String remoteDirChild = remoteDir + '/' + it.getName()
                    uploadFileOrDir(it, remoteDirChild, excludes)
                }
            }
        } else {
            assert local.isFile()
            putFile(local, remoteDir)
        }
    }

    void createDirSilent(String dir) {
        try {
            sftp.mkdir(dir)
        } catch (SftpException e) {
            if (e.id == com.jcraft.jsch.ChannelSftp.SSH_FX_FAILURE) {
                log.debug3("failed create dir ${dir} ", e)
            } else {
                log.info("failed create dir ${dir} ${e}")
                throw e
            }
        }
    }
    ...
    File rotateFileImplNS(String fileName1,int depth, int maxDepth) {
        int newDepth = depth + 1
        String fileFrom = buildRotateFileNS(fileName1, depth);
        String file2 = buildRotateFileNS(fileName1,  newDepth)
        SftpATTRS stat
        try {
            stat = sftp.stat(fileName1)
        }catch (SftpException e){
            log.info "file not found ${fileName1} : ${e}"
        }
        if (stat!=null) {
            if (newDepth > maxDepth) {
                sftp.rm(file2)
            } else {
                rotateFileImplNS(fileName1, newDepth, maxDepth);
            }
        }
        sftp.rename(fileFrom,file2)
    }

    public DecimalFormat decimalFormat1 = new DecimalFormat('00')

    String buildRotateFileNS(String fileName, int depth) {
        if (depth == 0) {
            return fileName;
        }
        synchronized (decimalFormat1) {
            return  "${fileName}.${decimalFormat1.format(depth)}"
        }
    }
