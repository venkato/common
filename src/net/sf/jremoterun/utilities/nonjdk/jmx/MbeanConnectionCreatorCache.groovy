...
import net.sf.jremoterun.utilities.nonjdk.apprunner.NetworkTcpPorts
...
class MbeanConnectionCreatorCache {
    ...
    static <T> T getClient(Class<T> clazz, JmxLocalhostConnections connection, ObjectName objectName) {
        return getClient(clazz,localhost,connection.getPort(),objectName)
    }
    ...
    static MbeanConnectionCreator getConnection(NetworkTcpPorts networkTcpPorts) {
        return getConnection(localhost,networkTcpPorts.getPort())
    }

    static MbeanConnectionCreator getConnection(JmxLocalhostConnections jmxLocalhostConnection) {
        return getConnection(localhost,jmxLocalhostConnection.getPort())
    }
    ...
    static MbeanConnectionCreator getLocalConnection(int port) {
        return getConnection(localhost, port);
    }
