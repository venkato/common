...
class BackupTransferHandler extends TransferHandler {
    ...
    public DropFileAware backupLocationJPanel;
...
//                    log.info "el1 cl = ${el1.getClass()}"
    ...
    public void exportToClipboard(JComponent comp, Clipboard clipboard, int action) throws IllegalStateException {
        if (comp instanceof JTextComponent) {
            ...
            if (p0 != p1) {
                exportToClipboardImpl(text, clipboard, action, p0, p1)
            }
        }
    }


    public void exportToClipboardImpl(JTextComponent text, Clipboard clipboard, int action, int p0, int p1) throws IllegalStateException {
        try {
