...
class Connection extends Thread {
    ...
    public MyTextArea inputText = new TcpMonTextArea(this, 20, 80);

    public RTextScrollPane inputScroll = null;

    public MyTextArea outputText = new TcpMonTextArea( this,20, 80);

    public RTextScrollPane outputScroll = null;

    public SocketI inSocket = null;

    public SocketI outSocket = null;
...
//        slowLink = l.slowLink;
    ...
    public Connection(final Listener l, final SocketI s,
...
*/
            ...
            if (count == 0 || lsm.getLeadSelectionIndex() == 0) {
                ...
                listener.lastSelectedConnection = this;
            ...
            if (tmpIn1 == null) {
                ...
                inputText.appendInSwingThread( inSocket.dumpSocketDebugInfo());
            }
            ...
            if (listener.isProxyBox.isSelected() || listener.tcpMonSettings.HTTPProxyHost != null) {
...
//							log.info("new proxy host "+targetHost);
            ...
            log.info(targetHost);
            outSocket = listener.sslTarget.createConnection(targetHost, targetPort);;
//            outSocket = new SocketWrapper( new Socket(targetHost, targetPort));
            ...
            outputText.appendInSwingThread( outSocket.dumpSocketDebugInfo());

            if (bufferedData != null) {
                final byte[] b = bufferedData.getBytes();
                tmpOut2.write(b);
                listener.tcpMonSettings. slowLink.pump(b.length);
            }
            ...
            rr1 = new SocketRR(this, inSocket, tmpIn1, outSocket, tmpOut2,
                    ...
                    listener.tcpMonSettings.slowLink, requestEncoding,
                    listener.changeHostcheckBox.isSelected() ? targetHost
                            + ":" + targetPort : null, listener.tcpMonSettings.showMsgData);
            ...
            final SlowLinkSimulator responseLink = new SlowLinkSimulator(
                    listener.tcpMonSettings.slowLink);
            ...
            rr2 = new SocketRR(this, outSocket, tmpIn2, inSocket, tmpOut1,
                    ...
                    requestEncoding, null, listener.tcpMonSettings.showMsgData);
            ...
            while (rr1 != null || rr2 != null) {
                ...
                if (null != rr1 && rr1.isDone()) {
                    if (index >= 0 && rr2 != null) {
                        listener.tableModel.setValueAt(
                                Tcpmon.getMessage("resp00", StateColumnValues.Resp.name()), 1 + index,
                                Tcpmon.STATE_COLUMN);
                    }
                    rr1 = null;
                }
                if (null != rr2 && rr2.isDone()) {
                    if (index >= 0 && rr1 != null) {
                        listener.tableModel.setValueAt(
                                Tcpmon.getMessage("req00",  StateColumnValues.Req.name()), 1 + index,
                                Tcpmon.STATE_COLUMN);
                    }
                    rr2 = null;
                }
            ...
            if (index >= 0) {
                listener.tableModel.setValueAt(
                        Tcpmon.getMessage("done00", StateColumnValues.Done.name()), 1 + index,
                        Tcpmon.STATE_COLUMN);
            ...
            if (index >= 0) {
                SwingUtilities.invokeLater(() -> listener.tableModel.setValueAt(
                        Tcpmon.getMessage("error00", StateColumnValues.Error.name()), 1 + index,
                        Tcpmon.STATE_COLUMN));
            }
            ...
            if (outputText != null) {
                ...
                log.log(Level.FINE,"failed "+listener.portField,e);
