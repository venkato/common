...
class EventQueueDebug extends EventQueue {
    ...
    EventQueueDebug() {
        ...
        seqNested = JrrClassUtils.findField(new ClRef('java.awt.SequencedEvent'), 'nested')
        inputMethod = JrrClassUtils.findField(new ClRef('sun.awt.windows.WInputMethod$1'), 'this$0')
...
//                println("4 null ${theEvent}")
                ...
                switch (className) {
                    case { className.startsWith(winWInputMethodClass.className) }:
    ...
    void printEventInfo(AWTEvent theEvent) {
        try {
            ...
            log.severe "exiting, failed handle ${theEvent} ${e}"
            e.printStackTrace()
            SystemExit.exit(1)
        }
            ...
            if (data == null) {
                ...
                switch (className) {
                    ...
                    case { className.startsWith(sun.awt.GlobalCursorManager.getName()) }:
                        ...
                        break;
                    case { className.startsWith(winWComponentPeerClass.className) }:
