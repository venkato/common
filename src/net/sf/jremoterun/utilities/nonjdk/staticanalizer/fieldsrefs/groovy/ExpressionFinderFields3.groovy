...
//            StaticFieldType staticFieldType = StaticFieldType.map1.get(text1)
    ...
    void visitMethodCallExpression(MethodCallExpression expression) {
        ...
        try {
            if (!ignoreLines.contains(expression.lineNumber) && isOkLine(expression.lineNumber)) {
            ...
            if (method1 instanceof org.codehaus.groovy.ast.expr.ConstantExpression) {
                ...
                if (type1 != null) {
                    ...
                    if (staticFieldType != null) {
                        ...
                        if (net.sf.jremoterun.utilities.nonjdk.staticanalizer.StaticFieldType.reFields.contains(staticFieldType )||
                                ...
                                , StaticFieldType.findMethodByParamTypes2 , StaticFieldType.findMethodByParamTypes3 , StaticFieldType.findMethodByParamTypes4 , StaticFieldType.runMainMethod]) {
                            ...
                            if (expressions1.size() > 1) {
                                ...
                                if (expression11 instanceof org.codehaus.groovy.ast.expr.ConstantExpression) {
                                    ...
                                    if (expression1Obj == null) {
                                        ...
                                        if (StaticFieldType.reFields.contains(staticFieldType)) {
                        ...
                        if (net.sf.jremoterun.utilities.nonjdk.staticanalizer.StaticFieldType.constructorsFunctions.contains(staticFieldType)) {
    ...
    ClRef resolveClRefExpression(Expression ee) {
        ...
        if (impl1 == null) {
            log.info "failed resolve line ${printablePath} ${ee.getLineNumber()} ${ee.getText()}"
        }
        return impl1
    }
    ...
    void visitConstructorCallExpression(ConstructorCallExpression expression) {
        ...
        try {
            if (!ignoreLines.contains(expression.lineNumber) && isOkLine(expression.lineNumber)) {
                String nameWithoutRef = expression.type.nameWithoutPackage
                StaticFieldType staticFieldType = StaticFieldType.map1.get(nameWithoutRef)
                if (net.sf.jremoterun.utilities.nonjdk.staticanalizer.StaticFieldType.memberRefClasses.contains(staticFieldType)) {
