...
//                loaderStuff.problemFound(elementMethodGroovy,text)
    ...
    void visitConstructorCallExpression3(ConstructorCallExpression expression) {
        ...
        if (staticFieldType == StaticFieldType.MethodRef || staticFieldType == StaticFieldType.FieldRef || staticFieldType == StaticFieldType.ConstructorRef) {
...
//            StaticFieldType staticFieldType = StaticFieldType.map1.get(text1)
            ...
            if (staticFieldType == StaticFieldType.FieldRef || staticFieldType == StaticFieldType.MethodRef || staticFieldType == StaticFieldType.ConstructorRef) {
    ...
    void onElementFound(Object obj1, Expression e) {
        interested.add new FieldFoundedEls(e, obj1)
    }

    private boolean isCheckType(MethodCallExpression expression) {
...
//                    if (getObjectExpression instanceof ClassExpression) {
        ...
        return name1 == JrrClassUtils.getName()
//                    }
    }
    ...
    void visitMethodCallExpressionImpl(MethodCallExpression expression) {
        ...
        if (arguments1 instanceof ArgumentListExpression) {
            ...
            if (method1 instanceof org.codehaus.groovy.ast.expr.ConstantExpression) {
                ...
                StaticFieldType staticFieldType = StaticFieldType.map1.get(text1)
                if (staticFieldType == StaticFieldType.setFieldValue || staticFieldType == StaticFieldType.getFieldValue || staticFieldType == StaticFieldType.findField || staticFieldType == StaticFieldType.invokeJavaMethod || staticFieldType == StaticFieldType.findMethodByCount || staticFieldType == StaticFieldType.findMethodByParamTypes1 || staticFieldType == StaticFieldType.findMethodByParamTypes2 || staticFieldType == StaticFieldType.findMethodByParamTypes3 || staticFieldType == StaticFieldType.findMethodByParamTypes4) {
                    if (isCheckType(expression)) {
                        if (expressions1.size() > 1) {
                            Expression expression11 = expressions1[1]
                            if (expression11 instanceof org.codehaus.groovy.ast.expr.ConstantExpression) {
                                ...
                                if (staticFieldType == StaticFieldType.setFieldValue || staticFieldType == StaticFieldType.getFieldValue || staticFieldType == StaticFieldType.findField) {
                                    onElementFound(new FieldRef(expression1Obj, constantExpression2.getText()), expression)
                                }
                                if (staticFieldType == StaticFieldType.invokeJavaMethod) {
                                    onElementFound(new MethodRef(expression1Obj, constantExpression2.getText(), expressions1.size() - 2), expression)
                                }
                                if (staticFieldType == StaticFieldType.findMethodByParamTypes1 || staticFieldType == StaticFieldType.findMethodByParamTypes3) {
                                    //Expression expression1Count = expressions1[2]
                                    //log.info "${expression1Count}"
                                    onElementFound(new MethodRef(expression1Obj, constantExpression2.getText(), expressions1.size() - 2), expression)
                                }
                                if (staticFieldType == StaticFieldType.findMethodByCount) {
                                    int counnttt = resolveInt(expressions1[2])
                                    onElementFound(new MethodRef(expression1Obj, constantExpression2.getText(), counnttt), expression)
                                }
                            }
                        }
                    }
                ...
                if (staticFieldType == StaticFieldType.invokeConstructor || staticFieldType == StaticFieldType.findConstructorByCount) {
                    if(isCheckType(expression)) {
                        Object expression1Obj = resolveClRefExpression(expressions1[0])
                        if (staticFieldType == StaticFieldType.findConstructorByCount) {
                            int counnttt = resolveInt(expressions1[1])
                            onElementFound(new ConstructorRef(expression1Obj, counnttt), expression)
                        } else {
                            onElementFound(new ConstructorRef(expression1Obj, expressions1.size() - 1), expression)
                        }
                    }
                }
            }
    ...
    ClRef resolveClRefExpression(Expression ee) {
        ...
        if (impl1 == null) {
            log.info "failed resolve line ${ee.getLineNumber()} ${ee.getText()}"
        }
        return impl1
    }
            ...
            if (name1 == 'java.lang.Class' || name1 == ClRef.getName()) {
                ...
                if (expression1 == null) {
                    ...
                    if (expression1 == null) {
                        ...
                        log.info("no init expression for ${variableExpression1.getText()}")
                        return null
                    }
                }
