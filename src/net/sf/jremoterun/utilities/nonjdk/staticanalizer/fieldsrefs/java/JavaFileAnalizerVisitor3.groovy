...
@CompileStatic
class JavaFileAnalizerVisitor3 extends VoidVisitorAdapter<Void> {
...
//    String cuClassName;
    ...
    @Override
    void visit(MethodCallExpr n, final Void arg){
        Optional<Expression> scope1 = n.getScope()
        if(scope1.isPresent()) {
            Expression expression = scope1.get()
            log.info "${expression.getClass().getName()}   ${expression}"
            if (expression instanceof NameExpr) {
                NameExpr nameExpr = (NameExpr) expression;
                SimpleName simpleName = nameExpr.getName()
                log.info "${simpleName}"
                if(simpleName.getIdentifier().contains(JrrClassUtils.getSimpleName())){
                    ResolvedValueDeclaration resolve1 = nameExpr.resolve()
                    log.info "res=${resolve1}"
                }
            }
        }
    }
...
//                        log.info "cp4 : ${fd}"
                        elementMethodGroovy.fieldType = StaticFieldType.ClRef;
