...
//            log.info "${el.fieldName} ${lineNo}"
        ...
        dir.listFiles().toList().each {
            ...
            try {
                if (it.isDirectory()) {
                    ...
                    assert f.file
                    if (!f.name.contains('$') && f.name.endsWith(ClassNameSuffixes.dotclass.customName)) {
    ...
    List<StaticElementInfoJavassist> analizeFile(File f) {
        assert f.name.endsWith(ClassNameSuffixes.dotclass.customName)
    ...
    List<StaticElementInfoJavassist> analizeJar(File jarFile) {
        ...
        try {
            ...
            while (entries.hasMoreElements()) {
                ...
                try {
                    if (!zipEnName.contains('$') && zipEnName.endsWith(ClassNameSuffixes.dotclass.customName)) {
    ...
    List<StaticElementInfoJavassist> analizeClass(ClassFile classFile) {
        ...
        fields.each {
            ...
            if (isFile || isCnr) {
                ...
                el.fieldInfo = fi;
                el.fieldType = isFile ? StaticFieldType.File : StaticFieldType.ClRef
                result.add(el)
            }
        }
        return result;
