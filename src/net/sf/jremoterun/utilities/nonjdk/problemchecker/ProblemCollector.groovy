...
//            }
    ...
    boolean checkIfProblemExistAndShowException() {
        ...
        if (size == 1) {
            ProblemInfo problemInfo = list[0]
            net.sf.jremoterun.utilities.JrrUtilitiesShowE.showException(problemInfo.msg,problemInfo.stackTrace)
        }
        if (size > 1) {
            net.sf.jremoterun.utilities.JrrUtilitiesShowE.showException(problemsSummary,new Exception(problemsSummary))
        }
        return false
    }
