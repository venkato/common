...
class FileRotate {
    ...
    static File buildRotateFile(File file, File archiveRotationDir, int depth) {
        if(depth==0){
            return file;
        }
        String dirSuffix;
...
//        String dirPrefix;
//        String newDepthString
        File fileFrom = buildRotateFile(file,archiveRotationDir,depth);
//        synchronized (decimalFormat1) {
//            //dirPrefix = "${file.name}.${decimalFormat1.format(depth)}"
//            //fileFrom = depth == 0 ? file : new File(archiveRotationDir, dirPrefix)
//            newDepthString = "${file.name}.${decimalFormat1.format(newDepth)}"
//        }
//        File file2 = new File(archiveRotationDir, newDepthString)
        File file2 = buildRotateFile(file,archiveRotationDir,newDepth)
        if (file2.exists()) {
