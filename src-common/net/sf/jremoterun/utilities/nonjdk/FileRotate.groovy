...
class FileRotate {
    ...
    void rotateFileNS(File file, File archiveRotationDir, int maxDepth) {
        if (file.exists()) {
            file = file.getAbsoluteFile().getCanonicalFile()
            rotateFileImplSyncNS(file, archiveRotationDir, 0, maxDepth, true, copyIfFailedRenameG)
        }
    }
    ...
    void rotateFileImplSyncNS(File file, File archiveRotationDir, int depth, int maxDepth, boolean doRenameOrCopy, boolean copyIfFailedRename) {
        if (doSync) {
            String syncOnObject = file.getAbsoluteFile().getCanonicalFile().getAbsolutePath().intern()
            synchronized (syncOnObject) {
                rotateFileImplNS(file, archiveRotationDir, depth, maxDepth, doRenameOrCopy, copyIfFailedRename, false)
            }
        } else {
            rotateFileImplNS(file, archiveRotationDir, depth, maxDepth, doRenameOrCopy, copyIfFailedRename, false)
        }
    ...
    void rotateFileImplNS(File file, File archiveRotationDir, int depth, int maxDepth, boolean doRenameOrCopy, boolean copyIfFailedRename, boolean newLogic) {
