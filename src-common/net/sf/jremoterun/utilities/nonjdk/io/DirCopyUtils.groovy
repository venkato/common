...
import net.sf.jremoterun.utilities.nonjdk.io.filesutils.CheckFreeSpace
...
//        createReport('skipped',skippedAll)
    ...
    void checkFreeSpace(File dest, long minSpace) {
        ...
        CheckFreeSpace.checkDiskFreeSpace(dest, minFreeSpaceInMg)
        CheckFreeSpace.checkDiskFreeSpaceInBytes(dest, minSpace * freeSpaceFactor)
    }
...
//        log.info "checking ${dest}"
    ...
    public boolean setExecPermission= true
    public String endsWithexecPermission='.sh'

    void setExecPermissionIfNeeded(File dest) {
        if(setExecPermission) {
            if (dest.getName().endsWith(endsWithexecPermission)) {
    ...
    boolean copyOneFileWithChecksImpl(File fromFile2, File dest, EolConversion convertEol) {
        if (convertEol != null && convertEol != EolConversion.asIs) {
            boolean b = copyOneFileEol(fromFile2, dest, convertEol)
            if (b) {
                setExecPermissionIfNeeded(dest)
            }
            return b
        }
        ...
        boolean b = copyOneFileImpl(fromFile2, dest)
        if (b) {
            setExecPermissionIfNeeded(dest)
        }
        return b;
    ...
    public long maxLengthForText = BytesDivider.mb.divider
