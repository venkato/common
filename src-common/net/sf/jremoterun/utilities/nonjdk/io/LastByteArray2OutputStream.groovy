...
@CompileStatic
class LastByteArray2OutputStream extends ByteArrayOutputStream {
...
//    private int lengthHalf;
    ...
    LastByteArray2OutputStream() {
        this(bufferSizeDefault)
    }

    LastByteArray2OutputStream(int bufferSize) {
        super(0)
...
//        lengthHalf = bufferSize / 2 as int;
    ...
    void ensureBytesFree(int byteFreeNeeded) {
        if (byteFreeNeeded > buf.length) {
            throw new IllegalStateException("needed=${byteFreeNeeded}, configured=${buf.length} ")
        }
        int freeCount = buf.length - count
...
//        usesFirst = !usesFirst
    ...
    synchronized void write(byte[] b, int off, int len) {
        if (len > buf.length) {
            int reduceCount = len - buf.length + 1
            off += reduceCount
            len = len - reduceCount
        }
        ensureBytesFree(len)
