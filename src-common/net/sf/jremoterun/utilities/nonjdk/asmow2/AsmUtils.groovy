...
class AsmUtils {
    ...
    AsmUtils(ClassLoader cl) {
        this.asmByteCodeVerifier = new AsmByteCodeVerifier(cl)
    }
    ...
    static String convertClassPath(ClRef fromClass) {
        return fromClass.className.replace('.', '/')
    }
    ...
    static byte[] readClass(Class clazz) {
        String path = convertClassPath(clazz) + ClassNameSuffixes.dotclass.customName
