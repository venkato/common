...
class CheckEach {
    ...
    Object onObject(FieldFoundedEls obj2) {
        ...
        if (finder.isFound()) {
            ResolvedFine resolvedFine = new ResolvedFine(obj2, classpathAnalysis.createCalculator().convertFileToObject(finder.foundField[0].baseFile))
            return resolvedFine
        } else {
            if(finder.classFound==null){
                return new ClassElNotFound(obj2.locationRef, obj2.foundEl)
            }else {
                return new ClassFoundButElementFailed(obj2.locationRef, obj2.foundEl,classpathAnalysis.createCalculator().convertFileToObject(finder.classFound.f))
            }
        }
    }
