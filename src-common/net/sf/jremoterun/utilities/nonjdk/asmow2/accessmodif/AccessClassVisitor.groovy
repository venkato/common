...
class AccessClassVisitor extends ClassVisitor {
    ...
    public static String contsructorMethodName = net.sf.jremoterun.utilities.nonjdk.asmow2.accessmodif.AsmConstains.constructorMethodName

    boolean needMakeAllPublic
    boolean needMakeAllPublicTmp
    AccessModifControllerP accessModifController
    ClassReader classReader
//    String innerClass

    AccessClassVisitor(ClassVisitor classVisitor, AccessModifControllerP accessModifController, ClassReader classReader) {
        super(AsmConsoleDecompiler.getMaxAsmM(), classVisitor);
...
//        log.info "handle ${}"
        ...
        if(!needPublic) {
            needPublic = accessModifController.needMakeFieldPublic(name, classReader)
        }
        if(needPublic){
