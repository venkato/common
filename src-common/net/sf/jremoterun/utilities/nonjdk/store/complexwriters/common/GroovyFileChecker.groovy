...
class GroovyFileChecker {
    ...
    //public ClassChecker2 classChecker2 = ClassChecker2.classChecker2

    static analize(String groovyText) {
    ...
    static List<ASTNode> analize(String script, CompilePhase compilePhase, boolean statementsOnly, boolean doStaticAnalisys) {
        ...
        GroovyCodeSource codeSource = new GroovyCodeSource(script, scriptClassName + ClassNameSuffixes.dotgroovy.customName, "/groovy/script")
...
//        compileTransformation.visit([cl],sourceUnit)
        if (doStaticAnalisys) {
            ...
            log.debug "${collector.hasErrors()}"
            log.debug "static analysis passed"
        }
