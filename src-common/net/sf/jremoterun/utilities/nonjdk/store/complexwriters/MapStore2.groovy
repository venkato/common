...
import net.sf.jremoterun.utilities.nonjdk.store.configloader.ConfigLoaderInstanceMap
...
//        writer3Importer.addImport(templateValueClass)
    ...
    static Map loadSettingsS(File file) {
        if (file.exists()) {
            return new ConfigLoaderInstanceMap<>(file,GroovyConfigLoaderJrr.configLoader).loadSettings()
        }
        return [:]
    }
