...
//        writer7Sub.body.add(new LineInfo(Brakets.netural, convertEl(obj)))
    ...
    void writeSourcesImpl(List sources) {
        ...
        writer7Sub.bodySuffixBlocks.add  new LineInfo(Brakets.openBacket,"if(${writer7Sub.varNameThis} instanceof ${AddFilesWithSourcesI.simpleName}){")
        writer7Sub.bodySuffixBlocks.add  new LineInfo(Brakets.netural,"  ${AddFilesWithSourcesI.simpleName} ${sourceVarName} = ${writer7Sub.varNameThis} as ${AddFilesWithSourcesI.simpleName} ${semiColumn}")
        int counttt = -1;
        sources.each {
            try {
                counttt++
