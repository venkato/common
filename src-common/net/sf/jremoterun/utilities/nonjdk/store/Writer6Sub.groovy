...
abstract class Writer6Sub extends Writer5Class {
    ...
    String getClassDeclarationName() {
        ...
        if(configClass2 == AddFilesToClassLoaderGroovy){
            return "class ${classNameGenerated} extends ${ClasspathConfigurator2.getSimpleName()}  {".toString()
        }
        return "class ${classNameGenerated} extends ${GroovyConfigLoader2.getSimpleName()}<${writeTemplate(configClass2)}> {".toString()
    }
    ...
    String getMainMethod() {
        ...
        return "void ${method.getMethod()}(${writeTemplate(getConfigClass())} ${varNameThis}){".toString()
    }
