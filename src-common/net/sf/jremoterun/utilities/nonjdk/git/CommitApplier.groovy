...
class CommitApplier {
     ...
     */
    static CommitApplier calcRebaseCommitList2(RevCommit branchCommit, RevCommit masterCommit, GitRepoUtils gitRepoUtils, int maxDepth,Date stopOnDate) {
        List<RevCommit> cherryPickList = new ArrayList<>();
        while (true){
            RevCommit parent = masterCommit.getParent(0)
            if(parent.name() == branchCommit.name()){
                cherryPickList.add(branchCommit)
                break
            }
            if(cherryPickList.size()>maxDepth){
                throw new Exception("Max depth reached ${masterCommit.name()}")
            }
            parent= gitRepoUtils.gitRepository.parseCommit(parent)
            Date date1 = GitTreeWalker.getCommitDate(parent)
            if(stopOnDate!=null && date1< stopOnDate){
                throw new Exception("Stop on date ${date1} ${masterCommit.name()} ")
            }
    ...
    DiffApplier createDiffApplier(RevCommit parent, RevCommit childCommit) {
        CommitIdCanonicalTreeParser iteratorParent = gitRepoUtils.getTreeIterator(parent)
        CommitIdCanonicalTreeParser iteratorChild = gitRepoUtils.getTreeIterator(childCommit)
    ...
    public doRollbackCommit = false

    void reApply(RevCommit revCommit) {
        RevCommit parent = revCommit.getParent(0)
        DiffApplier diff3
        if (doRollbackCommit) {
            diff3 = createDiffApplier(revCommit, parent);
        } else {
            diff3 = createDiffApplier(parent, revCommit);
        }
    ...
    void setCommitMsg(RevCommit revCommit, CommitCommand commitCommand, DiffApplier diff3) {
        ...
        autoM = autoM.sort()
        String rollbackMsg = doRollbackCommit ? 'RB: ' : ''
        String msg = "${rollbackMsg}${revCommit.getShortMessage()} . Original commit : ${revCommit.name()}. AutoMerged ${autoM.size()} ${autoM} Skipped entries ${manualPaths33.size()}  ${manualPaths33}"
        commitCommand.setMessage(msg)
    }
