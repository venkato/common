...
//    public List<DiffEntry> skipped = [];
    ...
    public boolean resolveConflictsAuto = true
...
//    public HashSet<String> touchedNow = new HashSet();
    ...
    DiffWriter createDiffWriter(){
        DiffWriter writer = new DiffWriter(entriesOriginal, gitRepoUtils)
        //log.info "diff : ${writer.convertDiffToText2()}"
        return writer
    }

    void logStat1(){
        Set<String> manualPaths = getManualPaths()
        if(manualPaths.size()==0){
            //log.info "no manual commits"
        }else {
            log.info "manual !! ${manualPaths}"
        }
        List<DiffEntry> mergedNoConflict = getByStatus(DiffStatusEnum.modifiedNoConflict)
        if(mergedNoConflict.size()>0){
            log.info "merged no conflict : ${mergedNoConflict}"
        }
        List<DiffEntry> modifiedAutoResolved = getByStatus(DiffStatusEnum.modifiedConflictResolvedAuto)
        if(modifiedAutoResolved.size()>0){
            log.info "merged auto resolved !! : ${modifiedAutoResolved}"
        }
    }

    void logStat2(){
        List<String>  s= diffStatus.collect {"${it.value} ${it.key}".toString()}
        s.each{
            log.info "${it}"
        }
    }
            ...
            try {
                getByStatus(DiffStatusEnum.modifiedConflictUnknown).each {
                    ...
                    if (f.length() > maxFileSizeToMergeInKb * 1000) {
                        ...
                        if (objectLoaderBase.getSize() > maxFileSizeToMergeInKb*1000 || objectLoaderTheir.getSize() > maxFileSizeToMergeInKb*1000) {
                            diffStatus.put(it, DiffStatusEnum.modifiedConflictFileTooBig)
                        } else {
                            MergeResult<RawText> res = resolveConflictAndApplyBase(it, db, objectLoaderBase, objectLoaderTheir)
...
// TODO now white space diff will be applied, even if asked to ignore
    MergeResult<RawText> resolveConflictAndApplyBase(DiffEntry entry, ObjectDatabase db, ObjectLoader objectLoaderBase, ObjectLoader objectLoaderTheir) throws IOException {
        ...
        RawText baseText = new RawText(objectLoaderBase.getCachedBytes());
        RawText ourText = new RawText(f.bytes);
        RawText theirsText = new RawText(objectLoaderTheir.getCachedBytes());
    ...
    void resolveConflictAndApply5(DiffEntry entry, MergeResult<RawText> merge) throws IOException {
        ...
        try {
            ...
            JrrIoUtils.closeQuietly2(ooo, log)
        }
    }
