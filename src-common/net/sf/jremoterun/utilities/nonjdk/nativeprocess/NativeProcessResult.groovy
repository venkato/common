...
// simple runner
    ...
    static NativeProcessResult runNativeProcessAndWait(String cmd, File runDir) {
        ...
        processResult.onTimeoutRunnable = {log.info "process running too long : ${cmd}"}
    ...
    void doWaitImpl() {
        try {
            ...
            if(onFinishedPrintFinished){
                out2.write "${JrrClassUtils.getCurrentClass().getName()} process finished with exit code ${exitCode} at ${new Date()} \n".getBytes()
                flushOutStreams()
            }
            if(onFinishedR!=null){
                flushOutStreams()
                try {
                    ...
                    log.log(Level.SEVERE,"failed run finisher",e)
                }
            }
            closeOutStreams()
            if (waitAsync) {
    ...
    protected void onTimeoutContinuesCheck(){
        while (true) {
            threadRunningImpl.join(timeoutInSec * 1000);
            flushOutStreams()
    ...
    @Override
    String toString() {
        return "alive=${threadRunningImpl.isAlive()} exitCode=${exitCode}, errStream=${errLast}, outStream=${outLast}";
    }
    ...
    void forceDestroy(){
        ...
        continueRunningOnTimeoutCheck = false
        String msg = "${JrrClassUtils.getCurrentClass().getName()}.forceDestroy() : ${new Date()} force closing ..\n"
