...
class NativeProcessResult {
    ...
    public SimpleDateFormat sdf = new SimpleDateFormat('dd HH:mm:ss')
...
// simple runner
    ...
    void doWaitImpl() {
        try {
            ...
            if(onFinishedPrintFinished){
                out2.write "${JrrClassUtils.getCurrentClass().getName()} ${buildAdditionalInfo()}process finished with exit code ${exitCode} at  ${getCurrentDateFormatted()} \n".getBytes()
                flushOutStreams()
            }
            if(onFinishedR!=null){
                flushOutStreams()
    ...
    void forceDestroy2(boolean excepionIfStopped){
        if(exitCode!=null){
            ...
            continueRunningOnTimeoutCheck = false
            String msg = "${JrrClassUtils.getCurrentClass().getName()}.forceDestroy() : ${getCurrentDateFormatted()} force closing ..${buildAdditionalInfo()}\n"
