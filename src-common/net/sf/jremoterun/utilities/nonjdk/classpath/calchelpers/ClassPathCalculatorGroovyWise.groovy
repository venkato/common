...
class ClassPathCalculatorGroovyWise extends ClassPathCalculatorGitRefSup {
    ...
    void calcMavenCache() {
        ...
        if (mavenCommonUtils.fileType == MavenFileType2.binary.fileSuffix) {
            if (grapeRepoHashToFileMap == null && mavenCommonUtils.mavenDefaultSettings.grapeFileFinder.getMavenLocalDir2() != null && mavenCommonUtils.mavenDefaultSettings.grapeFileFinder.getMavenLocalDir2().exists()) {
            ...
            if (mavenRepoHashToFileMap == null && mavenCommonUtils.mavenDefaultSettings.mavenFileFinder.getMavenLocalDir2() != null && mavenCommonUtils.mavenDefaultSettings.mavenFileFinder.getMavenLocalDir2().exists()) {
            ...
            if (gradleRepoHashToFileMap == null && mavenCommonUtils.mavenDefaultSettings.gradleFileFinder.getMavenLocalDir2() != null && mavenCommonUtils.mavenDefaultSettings.gradleFileFinder.getMavenLocalDir2().exists()) {
    ...
    MavenId tryFindMavenIdFromJar(File jarFile) {
        ...
        String shainFile = FileCheckSumCalc.calcSha1ForFile(jarFile)
        if (mavenId == null) {
            if (jarFile.file) {
            ...
            String shaInMaven = FileCheckSumCalc.calcSha1ForFile(fileInMaven)
    ...
    static void calcSha1ForFilePrint(File file) {
        assert file.exists()
        String sha1ForFile = FileCheckSumCalc.calcSha1ForFile(file)
        ...
        files.each {
            switch (it) {
                case { it instanceof File }:
                    ...
                    File file = (File) it;
                    if (file.isFile()) {
                        String hex = FileCheckSumCalc.calcSha1ForFile(file)
