...
//    MavenDependenciesResolver defaultMavenDepDownloader
    ...
    public
    static ToFileRef2 noMavenIdConfigJsonFile = new  FileToFileRef( new File(  net.sf.jremoterun.utilities.groovystarter.JrrStarterVariables2.getInstance().filesDir,"noMavenIds2.json"))
...
//        log.info "d ${res} ${file}"
        if (res == null) {
            ...
            if(mavenCommonUtils.isParent(MavenDefaultSettings.mavenDefaultSettings.grapeFileFinder.getMavenLocalDir2(), file)){
                return null
            }
            if(mavenCommonUtils.isParent(MavenDefaultSettings.mavenDefaultSettings.mavenFileFinder.getMavenLocalDir2(), file)){
                return null
            }
            if(mavenCommonUtils.isParent(MavenDefaultSettings.mavenDefaultSettings.gradleFileFinder.getMavenLocalDir2(), file)){
                return null
            }
