...
class GradleRepoHashToFileMap {
    ...
    void init(LongTaskInfo longTaskInfo) {
        Date start = new Date()
        mavenCommonUtils.mavenDefaultSettings.gradleFileFinder.getMavenLocalDir2().eachFileRecurse(FileType.FILES, {
            ...
            if (name.endsWith(".jar") && !name.endsWith("-sources.jar")) {
                ...
                MavenId mi = mavenCommonUtils.mavenDefaultSettings.gradleFileFinder.detectMavenIdFromFileName(it,false,mavenCommonUtils.fileType)
                if (mi != null) {
