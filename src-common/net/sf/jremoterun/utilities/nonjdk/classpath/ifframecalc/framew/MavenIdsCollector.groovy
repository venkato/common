...
//    public static File ifDirDefault
    ...
    public List<Class> checkClassExists = []
...
//    File ifDir
    ...
    MavenIdsCollector() {
        ...
        createClassLoaders()
        checkClassExists.add(JrrClassUtils)
    }
    ...
    List<MavenId> loadMavenIds2Impl(ClassPathMavenIds classPathMavenIds) {
        ...
        ContextClassLoaderWrapper.wrap2(parentLoader, {
            checkClassExists.each {
                try {
                    parentLoader.loadClass(it.getName())
                    throw new Exception("Class ${it.getName()} loaded by ${parentLoader}")
                } catch (ClassNotFoundException e) {
                    log.fine "ok not load : ${it.getName()}"
                }
            }
        });
        ContextClassLoaderWrapper.wrap2(loader, {
            checkClassExists.each {
                try {
                    loader.loadClass(it.getName())
                } catch (Throwable e) {
                    log.info "failed load ${it.getName()}"
                    throw e
                }
            }
