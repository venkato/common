    ...
    List<MavenId> loadMavenIds2Impl(ClassPathMavenIds classPathMavenIds) {
        ...
        ContextClassLoaderWrapper.wrap2(parentLoader, {
            checkClassExists.each {
                try {
                    parentLoader.loadClass(it.getClassName())
                    throw new Exception("Class ${it.getClassName()} loaded by ${parentLoader}")
                } catch (ClassNotFoundException e) {
                    log.fine "ok not load : ${it.getClassName()}"
                }
            }
        });
        ContextClassLoaderWrapper.wrap2(loader, {
            checkClassExists.each {
                try {
                    loader.loadClass(it.getClassName())
                } catch (Throwable e) {
                    log.info "failed load ${it.getClassName()}"
                    throw e
                }
            }
