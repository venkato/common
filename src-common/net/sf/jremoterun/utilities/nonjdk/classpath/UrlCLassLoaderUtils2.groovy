...
""".replace('\r\n', '\n').replace('\n', '\r\n')
    ...
    static List<File> getClassLocationAll2(final ClRef clazz,ClassLoader cll) {
        final String tailJava = clazz.getClassName().replace('.', '/') + ".class";
        final String tailGroovy = clazz.getClassName().replace('.', '/') + ".groovy";
        List<URL> urls = getClassLocationAll(clazz.getClassName(), cll)
        // log.info "urls = ${urls}"
    ...
    static File getClassLocationFirst(ClRef clazz,ClassLoader classLoader1) {
        ...
        if (all.size() == 0) {
            throw new FileNotFoundException("${clazz.getClassName()}")
        }
        return all.first();
    }
