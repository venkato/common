...
class MavenUploader {
     ...
     */
    public static String gilabTokenName = 'Private-Token'
    ...
    // gitlab repo url : https://gitlab.com/api/v4/projects/PRODJECTID/packages/maven
    // PRODJECTID see on main project page
...
//                log.log(Level.SEVERE, 'maven uploader failed', exception)
    ...
    File createPomFileAndAdd(MavenId mavenId) {
        String childPath = MavenPomUtils.buildPomSuffix(mavenId);
...
"""
    ...
    /**
     * For gitlab add custom header 'Private-Token': 'glpat-value
     * https://docs.gitlab.com/ee/user/packages/maven_repository/'
     *
     * That method seems sending only ..
     */
    ...
    void setGitlabToken(String token){
        setAuth(gilabTokenName,token)
    }
    ...
    void prepare() {
        session.setConfigProperty(ConfigurationProperties.HTTP_HEADERS, customHeaders)
