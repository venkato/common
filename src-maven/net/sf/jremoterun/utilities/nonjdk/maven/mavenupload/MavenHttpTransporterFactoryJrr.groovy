...
class MavenHttpTransporterFactoryJrr implements TransporterFactory {
    ...
    @Override
    HttpTransporterJrr newInstance(RepositorySystemSession session, RemoteRepository repository) throws NoTransporterException {
        Map<String, Object> configProperties = session.getConfigProperties()
        log.info "config keys = ${configProperties.keySet()}"
        HttpTransporterJrr transporter = new HttpTransporterJrr(repository,session)
        return transporter
    }
