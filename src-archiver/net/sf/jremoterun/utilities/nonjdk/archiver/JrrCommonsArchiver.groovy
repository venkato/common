...
//            log.info "cp1122"
    ...
    //public int DEFAULT_BUFFER_SIZE = BytesDivider.mb.dividerInt * 10;
//    public static Method fillMethod = JrrClassUtils.findMethodByCount(BufferedInputStream, 'fill', 0)
...
//        BufferedInputStream fin = new BufferedInputStream(new FileInputStream2(fileEntry), DEFAULT_BUFFER_SIZE)
//        fillMethod.invoke(fin,objects)
//        return fin
    }
    ...
    void createArchiveEntry(File file, String entryName, ArchiveOutputStream archive) throws IOException {
        try {
            createArchiveEntryImpl(file, entryName, archive)
        } catch (Throwable e) {
            log.info "failed create entry for ${file} ${e}"
            throw e
        }
    }

    void createArchiveEntryImpl(File file, String entryName, ArchiveOutputStream archive) throws IOException {
        if (needStop) {
            throw new Exception('stop requested')
        }
        lastFile = file;
        if (isFile1(file) || createEntryForDir) {
            ...
            if (entry.isDirectory()) {
                ...
                createArchiveEntryImpl2(entry, file, entryName, archive)
            }
            archive.closeArchiveEntry();
        }
    }

    void createArchiveEntryImpl2(ArchiveEntry entry, File file, String entryName, ArchiveOutputStream archive) throws IOException {
        InputStream input = null;
        try {
            lastWrittenFile = file
            // need first read first bites to check file is redable
            input = createInputStreamForEntry(file);
            archive.putArchiveEntry(entry);
            copy(entry, input, archive, copyBufferSize, file);
            processedFiles++;
        } catch (Throwable e) {
            //TODO write dummy bytes, so number of written bytes  matches file size
            onError(e, file, entryName, archive, entry)
        } finally {
            JrrIoUtils.closeQuietly2(input, log);
        }
    }
    ...
    protected long copy(ArchiveEntry entry, InputStream input, OutputStream output, int buffersize, File f) throws IOException {
        byte[] buffer = new byte[buffersize];

        int n;
        //long readcount=0;
        long size12 = entry.getSize()
        long count = 0;
        try {
            for (; -1 != (n = input.read(buffer));) {
                ...
                if (count > size12) {
                    ...
                    if (errorFiles.size() > maxErrorFiles) {
                        throw new Exception("size too big ${count} > ${size12}")
                    }
                    n = (int) (count - size12)
//                    throw new Exception("size too big ${count} > ${size12}")
                    if (n > 0) {
            ...
            onCopyError(eee, entry, input, output, count, f)
        }
        return count;
    }

    boolean fillBytesOnError = true

    void onCopyError(Exception eee, ArchiveEntry entryName, InputStream input, OutputStream output, long readCount, File file) {
        log.info("failed put ${entryName.getName()}", eee)
        if (fillBytesOnError) {
            if (errorFiles.size() > maxErrorFiles) {
                throw eee
            }
    ...
    void onError(Throwable e, File file, String entryName, ArchiveOutputStream archive, ArchiveEntry entry) {
        log.info("failed put ${entryName} ${file}", e)
        //archive.closeArchiveEntry();
        if (errorFiles.size() > maxErrorFiles) {
            throw e
        }
