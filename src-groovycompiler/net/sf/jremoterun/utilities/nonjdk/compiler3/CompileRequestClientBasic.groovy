...
@CompileStatic
class CompileRequestClientBasic implements CompilerRequest {
    ...
    static void detectIfDir() {
        ...
        ifDirDefault = GitSomeRefs.commonUtil.resolveToFile()
        //throw new Exception("Not found ifDir from ${list}")
    }

    //
    CompileRequestClientBasic() {
    }
    ...
    URLClassLoader createUrlClassLoader(){
        ClassLoader extClassLoader = findExtClassLoader()
        URLClassLoader urlClassLoader = new URLClassLoader(new URL[0], extClassLoader)
        return urlClassLoader
    }

    ClassLoader findExtClassLoader(){
        ClassLoader extClassLoader = ExtClassloaderFinder.extClassloaderFinder.findExtClassLoader()
    ...
    void init() {
        copyFilesTmp.filePrefix = 'groovyc-'
...
//        params.dirs.each { assert it.directory }
    ...
    GroovyCompilerReply compileImpl(GroovyCompilerParams params) {
        if(!params.eclipseCompiler){
            File toolsJarFile = new MavenCommonUtils().getToolsJarFile()
        ...
        addClassPathCopy JrrStarterJarRefs.jrrutilitiesOneJar
        addClassPathCopy JrrStarterJarRefs2.jremoterun.gitOriginRef()
        addClassPathCopy JrrStarterJarRefs2.jrrbasetypes.gitOriginRef()
//        adder.add JrrClassLocationRefs.JrrUtils1
        params.addTestClassLoaded ClRef
...
//            assert compileRequestRemoteClass.classLoader == loader
            ...
            result1 = service2.compile(params)
        })
    ...
    void doStdChecksForClass(ClRef clRef){
        try {
            ClassLoader classloaderParent = adderParent.getClassloader()
            ...
            throw new Exception("class ${clRef} found by parent classLoader")
