...
public class CustomRunnersView extends ViewPart {
	...
	public static volatile Text logTextField;
    ...
    public static ScriptLogInterface si = new ScriptLogInterface() {
       ...
       public void info(Object obj) {
        	Display.getDefault().asyncExec(new Runnable() {
				...
				@Override
				public void run() {
					logTextField.setText(logTextField.getText()+ " "+obj);
				}
            });
        }
    };
...
//		panel2.add(openFileButton);
	...
	static void refresh3() {
		try {
			...
			log.info(" " + e1);
			net.sf.jremoterun.utilities.JrrUtilitiesShowE.showException("Failed refresh", e1);
		}
	}
...
//		panel2.removeAll();
		...
		logTextField = new Text(panel2, SWT.PUSH);
		logTextField.setTextLimit(30);
				...
				openFileButton.addSelectionListener(new SelectionAdapter() {
					...
					public void widgetSelected(SelectionEvent e) {
						try {
							...
							osInegrationClient.openFile(file, CustomRunnerViewSettings.openCustimFileExetnstion);
						} catch (Throwable e1) {
							log.info(file.getName() + " " + e1);
							net.sf.jremoterun.utilities.JrrUtilitiesShowE.showException(file.getName(), e1);
						}
					}
				});
		...
		button.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				try {
					...
					Thread thread = new Thread(f.getName()+ " custom runner") {
						public void run() {
							try {
								log.info("file " + f.getName() + " calling ..");
								Object instance=net.sf.jremoterun.utilities.groovystarter.runners.GroovyConfigLoaderJrr.configLoader.parseConfig(f);
...
//								Object instance = clazz.newInstance();
								if(instance instanceof IdeScriptRunner){
									IdeScriptRunner ideScriptRunner = (IdeScriptRunner) instance;
									ideScriptRunner.run2(si);
			                    }else {
			                    	JrrClassUtils.invokeJavaMethod(instance, "run"); // NOFIELDCHECK
			                    }
								log.info("file " + f.getName() + " called");
							} catch (Throwable e) {
								log.info(f.getName() + " " + e);
								net.sf.jremoterun.utilities.JrrUtilitiesShowE.showException(f.getName(), e);
							} finally {
								Display.getDefault().asyncExec(new Runnable() {
					...
					net.sf.jremoterun.utilities.JrrUtilitiesShowE.showException(f.getName(), e2);
				}
			}
		});
		return button;
